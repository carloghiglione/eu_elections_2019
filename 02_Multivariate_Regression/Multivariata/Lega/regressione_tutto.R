...
max_cook <- max(as.numeric(cooks.distance(mod)[cok]))
ind_max_cook <- which(max_cook == as.numeric(cooks.distance(mod)))
senza_max <- as.numeric(cooks.distance(mod)[which(! 1:107 %in% ind_max_cook)])
max2 <- max(senza_max)
ind_max2 <- which(max2 == as.numeric(cooks.distance(mod)))

#vedo che il punto 32 è problematico, li aggiungo alla lista di punti da togliere
out_ind = unique(c(out_et_lev,out_et_cook,max_lev,ind_max2,ind_max_cook))
out_ind <- c(out_ind,32)
...
points(mod$fitted.values[ind_max_cook],rstandard(mod)[ind_max_cook], pch = 16, col='orange')
points(mod$fitted.values[ind_max2],rstandard(mod)[ind_max2], pch = 16, col='orange')
abline(h = 0, col='red')
abline(h = 2, col='blue')
abline(h = -2, col='blue')
...
points(mod$fitted.values[ind_max_cook],cooks.distance(mod)[ind_max_cook], pch = 16, col='orange')
points(mod$fitted.values[ind_max2],cooks.distance(mod)[ind_max2], pch = 16, col='orange')
